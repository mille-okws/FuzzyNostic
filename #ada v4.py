...
def get_memberships(febre, tosse, garganta, peito, ar=0, coriza=0, espirros=0, fadiga=0, cabeca=0):
    ...
    tosse_vals = {
        ...
        'resfriado':  fuzz.interp_membership(x_tosse, fuzz.trapmf(x_tosse, [0, 1, 2, 4]), tosse),
    ...
    # Fadiga
    fadiga_vals = {
        'gripe':       fuzz.interp_membership(x_fadiga, fuzz.trapmf(x_fadiga, [-0.5, 2, 3, 5.5]), fadiga),
        'resfriado':  fuzz.interp_membership(x_fadiga, fuzz.trapmf(x_fadiga, [-0.5, 1, 2, 3]), fadiga),
    ...
    # Dor de Cabeça
    dor_cabeca_vals = {
        'gripe':       fuzz.interp_membership(x_cabeca, fuzz.trimf(x_cabeca, [-0.5, 2, 5.5]), cabeca),
...
def diagnosticar():
    ...
    for doenca, grau in resultados_ordenados:
        output_text.insert(tk.END, f"{doenca.title()}: {grau*100:.2f}%\n")
